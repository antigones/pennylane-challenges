...
def variational_circuit(params,hamiltonian):
    ...
    parameters. To be used as a QNode, it must either be wrapped with the @qp.qnode decorator or
    converted using the qp.QNode function.
    ...
    qp.templates.StronglyEntanglingLayers(parameters, wires=range(WIRES))
    return qp.expval(qp.Hermitian(hamiltonian, wires = [0,1]))
...
def optimize_circuit(params,hamiltonian):
    ...
    circuit = qp.QNode(variational_circuit, dev)

    def cost(p):
        return circuit(p, hamiltonian)
    ...
    final = circuit(params, hamiltonian)
    return final # Return the value of the minimized QNode
